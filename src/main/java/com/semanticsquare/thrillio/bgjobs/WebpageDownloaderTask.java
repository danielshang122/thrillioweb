...
public class WebpageDownloaderTask implements Runnable {
	...
	private static final long TIME_FRAME = 3000000000L; // 3 sec
		...
		public T call() {
			try {
				if (!weblink.getUrl().endsWith(".pdf")) {
					weblink.setDownloadStatus(weblink.getDownloadStatus().FAILED);

					String htmlPage = HttpConnect.download(weblink.getUrl());
					weblink.setHtmlPage(htmlPage);
				} else {
					weblink.setDownloadStatus(weblink.getDownloadStatus().NOT_ELIGIBLE);
				}
